    ...
    <div class="fence-list" v-if="fences.length > 0">
      ...
      <div v-for="fence in fences" :key="fence.id" class="fence-item">
        <div class="fence-info">
          ...
          <span class="fence-type">{{
            fence.type === "rectangle" ? "矩形" : "圆形"
          }}</span>
        </div>
...
import type { GeoFence, BoundaryAlert, FenceType } from "../types/vehicle";
...
const emit = defineEmits<{
  (e: "close"): void;
  (e: "draw-start", type: "rectangle" | "circle"): void;
  (e: "draw-end", fence: Omit<GeoFence, "id" | "createdAt">): void;
  (e: "clear-fences"): void;
  (e: "delete-fence", fenceId: string): void;
}>();

const fences = ref<GeoFence[]>([]);
const alerts = ref<BoundaryAlert[]>([]);
const drawMode = ref<"rectangle" | "circle" | null>(null);
...
const startDrawCircle = (): void => {
  ...
  emit("draw-start", "circle");
};

/**
 * 添加围栏
 */
const addFence = (fence: GeoFence): void => {
