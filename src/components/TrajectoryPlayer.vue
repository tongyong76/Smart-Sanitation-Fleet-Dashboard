  ...
  <div class="trajectory-player" v-if="visible">
    ...
    <div class="player-content">
      ...
      <div class="time-range" v-if="trajectoryPoints.length > 0">
        ...
        <span class="value"
          >{{ formatTime(startTime) }} ~ {{ formatTime(endTime) }}</span
        >
      </div>
      ...
      <!-- 控制按钮 -->
      <div class="controls">
        <button class="control-btn" @click="play" :disabled="isPlaying">
          ▶ 播放
        </button>
        <button class="control-btn" @click="pause" :disabled="!isPlaying">
          ⏸ 暂停
        </button>
...
import type { TrajectoryPoint, Vehicle } from "../types/vehicle";
...
const emit = defineEmits<{
  (e: "close"): void;
  (e: "play-point", point: TrajectoryPoint): void;
}>();
