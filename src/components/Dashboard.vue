    ...
    <div class="dashboard-content" v-show="!collapsed">
      ...
      <div class="stats-row">
        <div class="stat-card-lg">
          ...
          <div
            class="stat-trend"
            :class="{ up: distanceTrend > 0, down: distanceTrend < 0 }"
          >
            {{ distanceTrend > 0 ? "↑" : "↓" }}
            {{ Math.abs(distanceTrend).toFixed(1) }}%
          </div>
        </div>
        <div class="stat-card-lg">
          ...
          <div class="stat-sub">
            在线: {{ onlineCount }} / 总: {{ totalVehicles }}
          </div>
        </div>
        <div class="stat-card-lg">
...
const updateStats = (): void => {
  ...
  onlineRate.value = Math.round(
    (onlineCount.value / totalVehicles.value) * 100,
  );
    ...
    series: [
      {
        ...
        data: [
          {
            name: "作业中",
            value: statusCount.sweeping,
            itemStyle: { color: "#10B981" },
          },
          {
            name: "闲置",
            value: statusCount.idle,
            itemStyle: { color: "#F59E0B" },
          },
          {
            name: "故障",
            value: statusCount.fault,
            itemStyle: { color: "#EF4444" },
          },
          {
            name: "充电",
            value: statusCount.charging,
...
const updateBatteryChart = (): void => {
  ...
  const counts = bins.map((_, i) => {
    if (i === bins.length - 1)
      return batteries.filter((b) => b >= bins[i]).length;
